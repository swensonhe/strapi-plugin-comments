...
export = ({ strapi }: StrapiContext): IServiceCommon => ({
  ...
  async checkBadWords(
    ...
    if (config) {
      if (content && isProfane({testString: content, options: config as any})) {
        throw new PluginError(
          400,
          "Bad language used! Please polite your comment...",
          {
            content: {
              original: content,
              filtered: content && replaceProfanities({testString: content, options: config as any}),
            },
          }
        );
      }
  ...
  async isEnabledCollection(
	...
	async fetchCommentsLikes(commentsIDs: Array<Id>) {
    return strapi.db.query("api::like.like").findMany({
      where: {
        commentID: {
          $in: commentsIDs,
        },
